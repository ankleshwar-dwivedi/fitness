...
class DashboardService {
  async generateTodaySummary(userId) {
    ...
    const [plan, todayMeals, todayWorkouts, todayWater] = await Promise.all([
      ...
      WaterLog.find({ user: userId, date: today }), // Fetch water logs
    ]);

      //  Instead of a fallout, we provide a default state.
    if (!plan) {
      return {
        hasPlan: false, // Add a flag for the frontend
        message: 'No fitness plan found. Please create one in your profile to see your dashboard.',
        // Provide default empty values for chart stability
        calorieGoal: 2000,
        caloriesConsumed: 0,
        caloriesBurned: 0,
        caloriesLeft: 2000,
