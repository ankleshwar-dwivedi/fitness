...
const WaterLogger = () => {
    ...
    return (
        <div className="max-w-md mx-auto text-center">
            {feedback.message && (
                <p className={`mb-4 font-semibold ${feedback.isError ? 'text-danger' : 'text-success'}`}>{feedback.message}</p>
            )}
            <h3 className="text-xl font-bold text-primary mb-4">Quick Add Water</h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {quickAddAmounts.map(amount => (
                    <Button key={amount} onClick={() => handleLogWater(amount)} className="w-full flex flex-col items-center !rounded-xl !py-4">
